            ...
            try:
                ...
                if last_seen_str:
                    ...
                    if silence_duration > 4 * deadline:
                        logger.warning("patient_heartbeat_silence_exceeded: silence=%fs limit=%fs appt_id=%s", silence_duration, 4 * deadline, appointment_id)
                        terminate = True
                        terminate_reason = "silence_timeout"
                    elif silence_duration > 2 * deadline:
                        logger.warning("patient_heartbeat_silence_degraded: silence=%fs limit=%fs appt_id=%s - restricting media", silence_duration, 2 * deadline, appointment_id)
        ...
        if not terminate and token_id and session_id and epoch is not None:
            try:
                ...
                if result:
                    status = int(result[0])
                    if status == -4:
                        logger.warning("patient_heartbeat_kill_switch_triggered_lua: appt_id=%s", appointment_id)
                        terminate = True
                        ...
                        if token_id == prev_token_str:
                            ...
                            logger.warning("patient_heartbeat_token_fenced_out: expected_active got=%s appt_id=%s", token_id, appointment_id)
                            terminate = True
                            terminate_reason = "token_mismatch"
                    elif status == -2:
                        logger.warning("patient_heartbeat_stale_epoch: incoming=%s stored_epoch=%s appt_id=%s", epoch, result[1], appointment_id)
                        terminate = True
                        terminate_reason = "stale_epoch"
                    elif status <= 0:
                        logger.warning("patient_heartbeat_lease_failed_status: status=%s appt_id=%s", status, appointment_id)
                        terminate = True
