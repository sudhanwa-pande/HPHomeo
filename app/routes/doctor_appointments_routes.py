...
async def doctor_call_heartbeat(
    ...
    if incoming_version is not None:
        stored_version_str = await redis.get_str(RedisKeys.session_version_key(appointment_id, "doctor"))
        stored_version = int(stored_version_str) if stored_version_str else db_version
        if incoming_version != stored_version:
            logger.info(
                "METRIC event=heartbeat_ignored action=ignored reason=session_version_mismatch db=%s incoming=%s appt_id=%s role=doctor session_id=%s",
    ...
    if session_id and seq == 0 and authority_mode == "redis":
        init_key = RedisKeys.init_key(appointment_id, "doctor", session_id)
        try:
            is_new = await redis.redis.setnx(init_key, "1")
            if is_new:
                await redis.redis.expire(init_key, 120)
                now = time.time()
                await redis.redis.setex(RedisKeys.last_seen_key(appointment_id, "doctor"), 120, str(now))
                if incoming_version is not None:
                    await redis.redis.setex(RedisKeys.session_version_key(appointment_id, "doctor"), 120, str(incoming_version))
                kill_version_key = RedisKeys.kill_version_key(appointment_id, "doctor", session_id)
